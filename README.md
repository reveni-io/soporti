...
Every tool above is read-only, with one exception you switch on yourself: with **write access** enabled (`/admin` → Shortcut), the assistant can file a Shortcut story, comment on one and change its fields from a conversation. It drafts the story from what the conversation actually established, shows it to you before writing anything, and **always asks whose name it goes under** — a Shortcut token belongs to one member, so it sets the story's Requester (and a comment's author) to the person you name instead of silently filing everything under the token's owner. Unattended runs — scheduled queries, the Slack ticket poller and the MCP endpoint — stay read-only, since nobody is there to answer that question.
...
- **Subagents** — define specialist agents in `/admin` → **Subagents**: each with its own system prompt, its own LLM provider and model, and its own set of tools. The assistant reads what each one owns and delegates on its own when a question belongs to a specialist, then answers from what comes back. A specialist runs in its own context, so a thirty-file code investigation never crowds the conversation. Its tools can be taken off the main assistant entirely — making delegation the only path to them, and the division of labour explicit — or left shared, in which case the specialist's description decides when the hand-off happens. The main assistant's own tools are editable too — from the graph you pick exactly which ones it may use, and leaving it on "every configured tool" keeps new integrations flowing to it automatically. A graph in `/admin` shows who owns what — every node carries one logo per integration it holds, and hovering a logo lists the exact tools — alongside the provider mark and model each one runs on and how many tools the main assistant has left. A delegation shows up in the chat as an ordinary step like any other tool call. Subagents are global and admin-only: there is nothing for a regular user to configure
...
- **Conversation history** — chats are persisted and reopenable from the sidebar. A new chat shows up there the moment you send the first message, titled with what you asked, and carries a live indicator while the answer is still being written — so a long investigation is visible in the list instead of appearing only once it finishes
- **Several conversations answering at once** — a long investigation no longer holds the app hostage: start a new chat or reopen an older one while it runs, and ask something else. Each conversation keeps its own answer streaming in, so leaving one stops watching it, not answering it — come back and the answer is there, finished or still arriving. The sidebar marks every conversation being answered and highlights the one you are reading, the composer is locked only for the conversation that is actually busy (one answer per conversation stays the rule), and **Stop** applies to the conversation on screen and nothing else
...
| `npm run docker:up:fresh` | Same, recreating the anonymous volumes — use it after adding a dependency, or the container keeps the old `node_modules` and the new package looks missing. Your database is a named volume, so it survives |
