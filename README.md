...
- **Attach documents and images to a message** — drag up to 3 files (`.pdf`, `.docx`, `.xlsx`, `.png`, `.jpg`, `.webp`, `.gif`, max 10 MB each) onto the composer, pick them with the paperclip, or paste a screenshot straight from the clipboard with `Cmd+V`. A document's text is extracted and given to the assistant as context, and a document longer than 50,000 characters is truncated and the message says so. An image is sent to the model as an image — a screenshot of an error or a photo of a broken screen is read, not described — and it is stored for 30 days (`CHAT_IMAGE_RETENTION_DAYS`) so a reloaded conversation still shows its thumbnail, after which the chip falls back to a placeholder. Attachments belong to their conversation only: they are not indexed into the knowledge base, are not visible from other chats, and their file names and images are hidden from read-only share links
