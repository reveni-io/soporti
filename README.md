...
- **Sign in with OAuth instead of a key** — the MCP endpoint is also an OAuth 2.1 authorization server, so a client can connect with no key at all: `claude mcp add --transport http soporti https://soporti.your.co/api/mcp` (no `--header`) opens the browser, you sign in with the login Soporti already has and approve the connection on a consent screen. Clients register themselves (RFC 7591), PKCE with S256 is mandatory, the access token is short-lived and bound to `https://soporti.your.co/api/mcp` as its audience — it is rejected anywhere else in the API — and the refresh token is rotated on every use, so replaying an old one revokes the whole grant. Discovery lives at `/.well-known/oauth-protected-resource/api/mcp` and `/.well-known/oauth-authorization-server`, and set `PUBLIC_URL` so both documents and the token audience carry the URL your clients actually reach. Existing `sop_…` keys keep working exactly as before
- **Claude plugin** — this repository doubles as a Claude plugin marketplace, so connecting Claude to the MCP server takes no terminal work and no key: add the marketplace (`/plugin marketplace add reveni-io/soporti` in Claude Code, or **Customize → Plugins → Add** with the repository URL in Cowork), install the **soporti** plugin and run `/soporti:setup`. The plugin declares the connector, whose endpoint comes from `SOPORTI_MCP_URL` so the package carries nobody's address; the command asks for yours, checks it really answers the OAuth challenge, and stores it. From then on the first question opens the browser, you sign in and approve the connection once. Anyone can install it — it needs no admin in Team or Enterprise, unlike a custom connector — and the server is registered namespaced as `plugin:soporti:soporti`, so it never collides with one you added by hand. See [`plugin/README.md`](plugin/README.md)
